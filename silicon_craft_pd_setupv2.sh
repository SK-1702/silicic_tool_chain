...
# OPENLANE_IMAGE="${OPENLANE_IMAGE:-efabless/openlane:master}"
...
ensure_docker_running() {
  ...
  if docker run --rm hello-world >/dev/null 2>&1; then
    ...
    die "Docker is installed but cannot run containers. Make sure the Docker service is running and try again."
  fi
}
...
setup_docker() {
  ...
  if ! command -v docker >/dev/null 2>&1; then
    warn "docker command not in PATH yet (new shell or re-login may be required). Trying with sudo docker..."
...
# If you *ever* want to debug PDN, you could temporarily:
#   set ::env(RUN_PDN) 0
# but for the installer we keep it enabled.
EOF
