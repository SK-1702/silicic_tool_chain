...
############################################
# Silicon Craft – Student PD Setup v12.1
# FINAL | Stable | PDK-correct | Instructor-approved
############################################

VLSI_ROOT="$HOME/Silicon_Craft_PD_Workspace"
OPENLANE_DIR="$VLSI_ROOT/OpenLane"
LOGFILE="$VLSI_ROOT/setup_v12_1.log"
...
info "Detected OS: $OS"
info "Silicon Craft – Student PD Setup v12.1"
...
wait_for_apt || die "APT lock did not clear. Close Software Updater and retry."

# ---------- APT Update + Auto Self-Heal ----------
info "==== Preflight: APT Update ===="
if ! sudo apt-get update; then
  warn "APT update failed (mirror/hash issue)."
  echo
  echo "This is common on fresh Ubuntu installs."
  echo "A safe APT self-heal can fix this."
...
if ! docker run --rm hello-world >/dev/null 2>&1; then
  warn "Docker installed but user permissions not active."
...
# ---------- Run OpenLane (NO manual PDK vars) ----------
...
info "✅ Silicon Craft PD Setup v12.1 COMPLETE"
